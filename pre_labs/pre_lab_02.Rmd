...
```{r}
courses_2025 <- umd_courses |> filter(term == 202408)
...
```{r}
theatre_seats_15 <- umd_courses |> filter(department == "Theatre" & seats >= 15)
...
**Answer** 54,000, now that it allows for a wider set of data.
...
**Answer:** The date is now formatted by year-month-date
...
**Answer** The data is now reflective of entire months instead of using specific dates.
...
**Answer** The code is calculating the percentage of votes for Moore and storing that number under the title "pct_moore".
...
str_to_sentence: Convert to sentence case, only first letter of sentence is capitalized.
...
**Answer** The mutate column is searching every data point for the address, detecting every state via a two syllable abbreviation, and ordering them by which state appears most often.
...
**Answer** It does. This is a Maryland election, so it makes sense that the majority of expenses come from the state. Virginia being as high as they are also makes sense given the proximity, although Washington (assuming it is the state) is curious.
...
1.  On average, what season has more classes offered? Summer, or winter?

2.  What is the combined percentage of Cox voters and write-in voters?

3.  What percentage of the vote did Moore get in Frederick County?
